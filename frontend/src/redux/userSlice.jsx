...
export const register=createAsyncThunk('user/register',async (userData,{rejectWithValue})=>{
 ...
 try {
        ...
        return rejectWithValue(error.message || 'Registration failed. Please try again later');
    }
...
export const login=createAsyncThunk('user/login',async ({email,password},{rejectWithValue})=>{
    try{    
     const response = await fetch('/api/v1/login', {
        ...
        const data = await response.json();
        if (!response.ok) {
            return rejectWithValue(data.message || JSON.stringify(data) || 'Login failed');
        }
        return data;
    }catch(error){
        return rejectWithValue(error.message || 'Login failed. Please try again later')
    }
})
...
export const loadUser=createAsyncThunk('user/loadUser',async(_,{rejectWithValue})=>{
    ...
    const data = await response.json();
    if (!response.ok) {
        return rejectWithValue(data.message || JSON.stringify(data) || 'Failed to load user profile');
    }
    return data;
} catch (error) {
...
export const updateProfile=createAsyncThunk('user/updateProfile',async(userData,{rejectWithValue})=>{
    try{
        ...
        const {data}=await axios.patch('/api/v1/profile/update',userData,config);
...
export const resetPassword=createAsyncThunk('user/resetPassword',async({token,userData},{rejectWithValue})=>{
    ...
    try {
    const response = await fetch(`/api/v1/reset/${token}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (!response.ok) {
        ...
        return rejectWithValue(data.message || { message: 'Reset password failed' });
    }
    return data;
} catch (error) {
    return rejectWithValue({ message: error.message || 'Reset password failed' });
}
})
